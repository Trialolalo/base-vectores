...
async function example() {
  ...
  let url = `http://www.clubtokyo.org/site/currentVariationList`;
  ...
  for (let offerUrl of offersUrl){
    let url = `http://www.clubtokyo.org/site/showVariation?fromPage=CURRENT_VARIATION&entryId=${offerUrl}`
    await driver.get(url);
  ...
  fs.writeFileSync('clubtokyo-variation.json', JSON.stringify(offers, null, 2));
}
